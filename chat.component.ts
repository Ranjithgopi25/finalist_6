...
export class ChatComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  async sendMessage(): Promise<void> {
    ...
    if (quickStartAwaitingFeedback) {
      ...
      // Route to draft workflow service for satisfaction handling
      this.draftWorkflowService.handleDraftSatisfaction(trimmedInput);
      return;
    }
    
    if (isAwaitingFeedback) {
      console.log('[ChatComponent] *** HANDLING DRAFT FEEDBACK - Input: "' + trimmedInput + '" ***');
      const satisfactionResult = this.analyzeDraftSatisfactionResponse(trimmedInput);
