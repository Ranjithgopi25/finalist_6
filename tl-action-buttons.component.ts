...
sendToChat(): void {
  ...
  private isEditContentWithHtml(): boolean {
    if (!this.message?.isHtml || !this.message?.content) {
      return false;
    }
    
    const content = this.message.content;
    // Check for HTML formatting from formatFinalArticleWithBlockTypes
    // Look for styled elements (h1 with style, p with style, ul with style)
    return content.includes('<h1 style') || 
           content.includes('<h2 style') ||
           content.includes('<p style') ||
           content.includes('<ul style') ||
           content.includes('revised-content-formatted');
  }
